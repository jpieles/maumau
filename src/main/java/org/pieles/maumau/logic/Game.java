...
public class Game {
    ...
    private void playerTurn(int input) {
            ...
            if (input > 0 && input <= manager.getPlayerCards().size()) {
                if (check(manager.getPlayerCards().get(input - 1), ListUtil.getLast(manager.getDeskCards()))) {
                    ...
                    if (ListUtil.getLast(manager.getDeskCards()).contains("8")) {
                        System.out.println("Der nächste Spieler muss eine Runde aussetzen");
                        aussetzen = true;
                    }
    ...
    /**
     * Wenn bereits ein Bube liegt, darf kein Bube gelegt werden -> nur true,
     * wenn die gelegte Karte kein Bube ist<br />
